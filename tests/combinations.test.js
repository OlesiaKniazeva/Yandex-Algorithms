const { combine } = require('../src/algorithms/combinations');

describe('combine', () => {
  test('should return the correct combinations', () => {
    expect(combine(4, 2)).toEqual([[1, 2], [1, 3], [1, 4], [2, 3], [2, 4], [3, 4]]);
    expect(combine(5, 3)).toEqual([[1, 2, 3], [1, 2, 4], [1, 2, 5], [1, 3, 4], [1, 3, 5], [1, 4, 5], [2, 3, 4], [2, 3, 5], [2, 4, 5], [3, 4, 5]]);
    // Add more test cases as needed
  });

  test('should return an empty array if k is 0', () => {
    expect(combine(5, 0)).toEqual([]);
  });

  test('Should return an empty array if n < k', () => {
    expect(combine(2, 4)).toEqual([]);
  })

  // Add more test cases to cover different scenarios and edge cases
});
